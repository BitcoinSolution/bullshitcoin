...
uint256 hashGenesisBlock("0x63312d140156ad40668a89286cabf8f86a689ab6df5d318b314b8c3d1edae58e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BullshitCoin: starting difficulty is 1 / 2^12
...
{
    // BullshitCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BullshitCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BullshitCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BullshitCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x63312d140156ad40668a89286cabf8f86a689ab6df5d318b314b8c3d1edae58e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1394805329;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084580019;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BullshitCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BullshitCoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running BullshitCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BullshitCoinMiner terminated\n");
        throw;
    }
}
